function orthoBiHarm = orthoplate_biharm_build(Evec,nux,Lvec,hvec,Nvec,KRmat)
...
for m = 3 : Nx - 1
    orthoBiHarm((Ny+1)*(m-1)+1 : (Ny+1)*m, (Ny+1)*(m-1)+1 : (Ny+1)*m)        = Blk33 ;
    orthoBiHarm((Ny+1)*(m-1)+1 : (Ny+1)*m, (Ny+1)*(m-2)+1 : (Ny+1)*(m-1))    = Blk32 ;
    orthoBiHarm((Ny+1)*(m-1)+1 : (Ny+1)*m, (Ny+1)*(m-3)+1 : (Ny+1)*(m-2))    = Blk31 ;
    orthoBiHarm((Ny+1)*(m-1)+1 : (Ny+1)*m, (Ny+1)*m+1 : (Ny+1)*(m+1))        = Blk34 ;
    orthoBiHarm((Ny+1)*(m-1)+1 : (Ny+1)*m, (Ny+1)*(m+1)+1 : (Ny+1)*(m+2))    = Blk35 ;
end

orthoBiHarm(1:Ny+1,1:Ny+1)           = Blk11 ;
orthoBiHarm(1:Ny+1,Ny+2:2*Ny+2)      = Blk12 ;
orthoBiHarm(1:Ny+1,2*Ny+3:3*Ny+3)    = Blk13 ;


orthoBiHarm(Nx*(Ny+1)+1:(Ny+1)*(Nx+1),Nx*(Ny+1)+1:(Ny+1)*(Nx+1))         = BlkMM ;
orthoBiHarm(Nx*(Ny+1)+1:(Ny+1)*(Nx+1),(Nx-1)*(Ny+1)+1:(Ny+1)*Nx)         = BlkMMm1 ;
orthoBiHarm(Nx*(Ny+1)+1:(Ny+1)*(Nx+1),(Nx-2)*(Ny+1)+1:(Ny+1)*(Nx-1))     = BlkMMm2 ;


orthoBiHarm(Ny+2:2*Ny+2,1:Ny+1)          = Blk21 ;
orthoBiHarm(Ny+2:2*Ny+2,Ny+2:2*Ny+2)     = Blk22 ;
orthoBiHarm(Ny+2:2*Ny+2,2*Ny+3:3*Ny+3)   = Blk23 ;
orthoBiHarm(Ny+2:2*Ny+2,3*Ny+4:4*Ny+4)   = Blk24 ;

orthoBiHarm((Ny+1)*(Nx-1)+1:Nx*(Ny+1),Nx*(Ny+1)+1:(Ny+1)*(Nx+1))         = BlkMm1M ;
orthoBiHarm((Ny+1)*(Nx-1)+1:Nx*(Ny+1),(Nx-1)*(Ny+1)+1:(Ny+1)*(Nx))       = BlkMm1Mm1 ;
orthoBiHarm((Ny+1)*(Nx-1)+1:Nx*(Ny+1),(Nx-2)*(Ny+1)+1:(Ny+1)*(Nx-1))     = BlkMm1Mm2 ;
orthoBiHarm((Ny+1)*(Nx-1)+1:Nx*(Ny+1),(Nx-3)*(Ny+1)+1:(Ny+1)*(Nx-2))     = BlkMm1Mm3 ;
